...
let postlist = function(select_list, page_num, callback){ //게시글 목록 로드(확인 필요)
    const onepage = 10; //한 페이지 최대 게시글 수 20->10
...
module.exports = {
    my_list : function(req, callback){
        req.conn.execute(`select * from postlist where id = '${req.session.user_id}' order by savedate desc`, [], [], function(err, list_post){
            postlist(list_post, req.query.page, function(maxpage, list_page){
                ...
                callback(err, maxpage, req.query.page, list_page, list_post.rows.length);
            })
        });
    },
    insert_post : function(req, callback){
        req.conn.execute(`insert into postlist values ('${req.session.user_id}', '${req.body.url}', 'path_pdf', to_char(sysdate, 'YYYY-MM-DD'), '${req.body.tag1}', 
        '${req.body.tag2}', '${req.body.tag3}', '${req.body.tag4}', '${req.body.tag5}', '${req.body.tag6}', '${req.body.tag7}', '${req.body.tag8}', 
        '${req.body.tag9}', '${req.body.tag10}', '${req.body.title}', post_num.nextval, '${req.body.text}', sysdate)`, [], [], function(err, insert){
    ...
    update_post : function(req, callback){        
        ...
        tag7='${req.body.tag7}', tag8='${req.body.tag8}', tag9='${req.body.tag9}', tag10='${req.body.tag10}', savedate_char = to_char(sysdate, 'YYYY-MM-DD'),
        savedate = sysdate where post_num = ${req.body.post_num}`, [], [], function(err, update){
